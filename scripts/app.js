...
search.addEventListener('keypress', (e) => {
    if(event.key === 'Enter'){
        ...
        for(let i=0; i < recentArr.length; i++){
            ...
            listItem.innerHTML = `<span>${recentArr[i]}</span>`;
            let saveBtn = document.createElement('button');
            saveBtn.type = 'button';
            saveBtn.className = "saveBtn";
            saveBtn.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-star" viewBox="0 0 16 16">
